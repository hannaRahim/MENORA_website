...
// Function to fetch the next available ebook_id
async function getNextEbookId() {
    try {
        // Fetch the maximum ebook_id, ordered in descending order to get the last one
        const { data, error } = await db.from('ebook')
            .select('ebook_id')
            .order('ebook_id', { ascending: false })
            .limit(1);

        if (error) {
            console.error('Error fetching max ebook_id:', error);
            return 1; // Default to 1 if there's an error or no books
        }

        if (data && data.length > 0) {
            // Assuming ebook_id is a number. If it's a string, you might need parseInt
            const lastId = parseInt(data[0].ebook_id, 10);
            return lastId + 1;
        } else {
            return 1; // If no books exist, start with ID 1
        }
    } catch (error) {
        console.error('Unexpected error in getNextEbookId:', error);
        return 1; // Fallback in case of unexpected errors
    }
}


// Marked this function as async
async function populateAddBookForm(event) {
    const itemRow = event.target.closest('.wishlist-item');
    
    if (!itemRow) {
        console.error("Could not find the parent '.wishlist-item' for the clicked element.");
        return; // Exit the function if itemRow is null
    }
    ...
    // Fetch and set the next ebook_id
    const nextId = await getNextEbookId();
    bookIdInput.value = nextId;
    bookIdInput.readOnly = true; // Make the input field read-only
...
async function addBookFromWishlist(event) {
    ...
    const newBook = {
        // IMPORTANT: Ensure ebook_id is NOT included in the object sent to Supabase
        // Supabase will automatically assign the next auto-increment ID.
        ...
        genre: document.getElementById('genre').value || null, // This will now correctly get the selected value from the dropdown
    };
    // Validate that title is present, as ebook_id is now auto-generated by Supabase
    if (!newBook.title) {
        showConfirmationModal('Validation Error', 'Title is a required field.');
        return;
    }
    try {
        // Do not include ebook_id in the insert. Supabase handles it.
        // The insert call should look like this:
        const { error: insertError } = await db.from('ebook').insert([newBook]);
        if (insertError) {
            // If there's a unique constraint error on another column (e.g., title if it's unique)
            if (insertError.code === '23505') {
                throw new Error(`Error adding book: A book with similar details might already exist.`);
            }
